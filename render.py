...
def draw(stdscr, width, height, ground_y, p_units, e_units, gold, p_hp, e_hp, 
         army_lv, _unused_dmg_lv, upgrade_cost, _unused_dmg_cost, show_promo_mode, s_type, a_type, k_type, ai_s, ai_a, ai_k):
    """
    ...
    # AI 보병 정보
    s_name = "Soldier" if ai_s == "#" else ("Spearman" if ai_s == "S" else "Paladin")
    stdscr.addstr(2, ai_info_x, f"Inf: {ai_s} ({s_name})", curses.color_pair(2))
    
    # AI 궁수 정보
    a_name = "Archer" if ai_a == "&" else ("Musketeer" if ai_a == "M" else "Javelin")
    stdscr.addstr(3, ai_info_x, f"Arc: {ai_a} ({a_name})", curses.color_pair(2))
    
    # AI 기병 정보
    k_name = "Knight" if ai_k == "@" else ("Chariot" if ai_k == "C" else "W.Hussar")
    stdscr.addstr(4, ai_info_x, f"Cav: {ai_k} ({k_name})", curses.color_pair(2))
    ...
    for u in p_units + e_units:
        ...
        char = u.kind
        if u.kind in ["&", "M", "J"] and u.state_timer > 0:
            char = "$" # 원거리 공격 연출
    ...
    if show_promo_mode > 0:
        ...
        if show_promo_mode == 1:
            ...
            stdscr.addstr(start_y + 3, start_x + 3, "[6] Spearman: DMG 7, Range 2 (4G)", curses.color_pair(1))
            ...
            stdscr.addstr(start_y + 3, start_x + 3, "[6] Musketeer: DMG 10, Range 6 (8G)", curses.color_pair(1))
            ...
            stdscr.addstr(start_y + 4, start_x + 3, "[7] W.Hussar: DMG 18, Range 3 (18G)", curses.color_pair(1))
