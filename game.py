...
def run(stdscr):
    ...
    # --- 아래 3줄이 누락되어 골드와 쿨다운이 멈춘 상태입니다 ---
        ...
        if key == ord("9") and eco.gold >= upgrade_cost:
            ...
            army_level += 1
            upgrade_cost = int(upgrade_cost * 1.8)  # 강화할수록 비용 상승
        # 6. 생산 (AI)
        ...
        for i, u in enumerate(player_units):
            ...
            if not stop:
                u.x += SPEED * 1.8 if u.kind == "@" else SPEED
