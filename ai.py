...
def ai_spawn(ai_units, ai_gold, spawn_x, start_time):
    ...
    # --- 1분마다 랜덤 전직 ---
    if elapsed >= 60 and ai_current_types["soldier"] == "#":
        ai_current_types["soldier"] = random.choice(["S", "P"])
    if elapsed >= 120 and ai_current_types["archer"] == "&":
        ai_current_types["archer"] = random.choice(["M", "J"])
    if elapsed >= 180 and ai_current_types["knight"] == "@":
        ai_current_types["knight"] = random.choice(["C", "W"])
